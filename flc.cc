...
private:
    ...
    int **rules;
    cOutVector qtime, hpUserWeight[100];
...
void FLC::initialize() {
    ...
    hpUsersIndex = (int*) malloc(sizeof(int) * usersCount);
    for (int counter = 0; counter < usersCount; counter++)
        if (getParentModule()->getSubmodule("user", counter)->par(
                "radioLinkQuality").intValue() == 4) {
            hpUsersIndex[hpUsersCount] = counter;

            std::string vecName = "User ";
            vecName += std::to_string(counter);
            vecName += " weight vector.";
            hpUserWeight[hpUsersCount].setName(vecName.c_str());
